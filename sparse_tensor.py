...
class SparseJacobian:
    def __init__(
        ...
        self.sparse_jacobians = sparse_jacobians or SparseJacobian.map_on_batch(
            lambda arg: SparseJacobian._get_sparse_typeclass(arg[1])(arg[0], arg[1], self.shape),
    ...
    def to_sparse_jacobian(cls, batched_jacobian):
        ...
        sparse_jacobians = SparseJacobian.map_on_batch(SparseJacobian._to_sparse, batched_jacobian)
        indices = SparseJacobian.map_on_batch(lambda x: x.coalesce().indices(), sparse_jacobians)
        values = SparseJacobian.map_on_batch(lambda x: x.coalesce().values(), sparse_jacobians)
    ...
    def __add__(self, other):
        ...
        if isinstance(other, SparseJacobian):
            ...
            added_jacobians = SparseJacobian.map_on_batch(
                lambda x: other + x, self.sparse_jacobians, self.batch_size
            )
    ...
    def inverse(self):
      ...
      if self.shape[0] != self.shape[1]:
        raise ValueError(f"Cannot invert matrix of shape: [{self.shape[0]}, {self.shape[1]}]. Need a square matrix")
      denses = SparseJacobian.map_on_batch(inverse_fn, self.sparse_jacobians, self.batch_size)
    ...
    def to_dense(self):
        ...
        denses = SparseJacobian.map_on_batch(to_dense_fn, self.sparse_jacobians, self.batch_size)
